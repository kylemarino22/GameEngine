...
//    }
    ...
    public FloatBuffer genUniformArrays (FloatBuffer uniformFloats) {

        for(Pair<Object, Integer> uniform : uniformList) {

            if (uniform.fst instanceof Matrix4f) {
                ((Matrix4f) uniform.fst).store(uniformFloats);
            }
            else if (uniform.fst instanceof Vector3f) {
                ((Vector3f) uniform.fst).store(uniformFloats);
            }
            else if (uniform.fst instanceof Float) {
                uniformFloats.put((Float) uniform.fst);
            }
        }
        uniformFloats.flip();
